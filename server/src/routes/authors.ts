...
export const createAuthorsRouter = (pool: Pool) => {
  const router = Router();
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ error: "Server error fetching author." });
    }
  });
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ error: "Server error creating author." });
    }
  });
